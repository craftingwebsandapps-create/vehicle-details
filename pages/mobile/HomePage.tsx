    ...
    <div className="space-y-6">
      <section className="rounded-[28px] bg-primary p-5 text-primary-foreground shadow-[0_18px_48px_-28px_rgba(234,88,12,0.5)]">
        ...
        <p className="mt-3 text-sm leading-6 opacity-90">
          Track live orders, run quick lookups, and keep essential actions within thumb reach.
        </p>
      </section>
      ...
      <section className="grid grid-cols-3 gap-3">
        {insightCards.map((item) => {
          ...
          return (
            <article key={item.label} className="rounded-[24px] border border-border/60 bg-background p-4 shadow-sm">
              <Icon className="size-5 text-primary" />
              <p className="mt-4 text-xl font-semibold text-foreground">{item.value}</p>
